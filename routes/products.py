...
def product_slug_meta(product_slug):
    try:
        ...
        # Check if client wants JSON (your frontend) or HTML (social media crawlers)
        accept_header = request.headers.get('Accept', '')
        user_agent = request.headers.get('User-Agent', '')
        
        is_crawler = any(bot in user_agent.lower() for bot in [
            'facebookexternalhit', 'twitterbot', 'whatsapp', 'telegram', 'linkedinbot', 
            'pinterest', 'slackbot', 'discord', 'googlebot'
        ])
        
        # Return JSON for API clients
        if 'application/json' in accept_header and not is_crawler:
            return jsonify({
                "product_id": product.product_id,
                "name": product.name,
                "description": product.description,
        ...
        # Return HTML for crawlers or if client accepts HTML
        meta_html = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
